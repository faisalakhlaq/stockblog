...
@stock.route('/stock_technical_terms', methods=['GET'])
def stock_technical_terms():
    technical_terms = StockTechnicalTerms.query.all()
    return render_template("stock_technical_terms.html", terms=technical_terms)
        ...
        if request.form.get('clear') or request.form.get('refresh'):
            ...
            if form.validate_on_submit():
                try:
                    ...
                    db.session.add(data)
                    db.session.commit()
                    flash(f"Created a new term {term_name}")
                    return render_template(
                        "stock_terms_entry_form.html",
                        terms=technical_terms,
                        form=StockTechnicalTermsForm(None)
